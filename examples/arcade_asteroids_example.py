...
class Player(arcade.Sprite):
    ...
    def on_update(self, delta_time: float = 1 / 60):
        if self.easing_angle_data is not None:
            done, self.angle = arcade.ease_angle_update(
                self.easing_angle_data, delta_time)
            if done:
                self.easing_angle_data = None

        if self.easing_x_data is not None:
            done, self.center_x = arcade.ease_update(
                self.easing_x_data, delta_time)
            if done:
                self.easing_x_data = None

        if self.easing_y_data is not None:
            done, self.center_y = arcade.ease_update(
                self.easing_y_data, delta_time)
            if done:
                self.easing_y_data = None
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key in [arcade.key.KEY_6, arcade.key.KEY_7, arcade.key.KEY_8, arcade.key.KEY_9]:
            ...
            ex, ey = arcade.ease_position(
                p1, p2, rate=180, ease_function=ease_function)
